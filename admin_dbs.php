...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $status = $_POST['signatory_status'] ?? null;
    $comments = $_POST['comments'] ?? '';
    $proposal_id = $_POST['proposal_id'] ?? null;

    if (!$proposal_id || !$approval_id || !$status) {
        addFlashMessage("Missing required data.", 'danger');
    ...
    if ($status === 'Approved') {
        ...
        $approved_at = empty($existing_approved_at) ? date('Y-m-d H:i:s') : $existing_approved_at;
    ...
    if ($stmt->execute()) {
        addFlashMessage("Signatory status and comments updated successfully.", 'success');
        ...
        // Reset if marked for revision
        if ($status === 'Revise') {
            $resetStmt = $connection->prepare("
                UPDATE tbl_proposal_signatories SET signatory_status = 'Pending' WHERE proposal_id = ?
            ");
            $resetStmt->bind_param('i', $proposal_id);
            if ($resetStmt->execute()) {
                addFlashMessage("All signatory statuses reset to Pending. Proposal marked for revision.", 'warning');
                ...
                if ($reviseStmt->execute()) {
                    addFlashMessage("Proposal status updated to Revise.", 'info');
                } else {
                    addFlashMessage("Failed to update proposal status to Revise: " . $reviseStmt->error, 'danger');
                }
                $reviseStmt->close();
                ...
                addFlashMessage("Failed to reset signatory statuses: " . $resetStmt->error, 'danger');
            }
            $resetStmt->close();
        ...
        if ($updateStmt->execute()) {
            addFlashMessage($msgText, $msgType);
        } else {
            addFlashMessage("Error updating proposal status: " . $updateStmt->error, 'danger');
        }
        $updateStmt->close();
    } else {
        addFlashMessage("Error updating signatory status: " . $stmt->error, 'danger');
    }
...
<body>
                    ...
                    <div class="tab-content">                   
                        ...
                        role="tabpanel" aria-labelledby="evaluation-tab">
                            ...
                            <div class="card-body">
                                ...
                                // Check if a proposal_id was provided in the URL
                                $selected_proposal_id = isset($_GET['proposal_id']) ? intval($_GET['proposal_id']) : 0;
                                
                                // Get proposals with evaluations
    ...
    async function filterData() {
        ...
        const url = new URL('../03_admin/chart_data_ajax.php', window.location.href);
