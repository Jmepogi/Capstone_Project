...
// The proposal ID will be passed via GET parameter
...
// If accessed via AJAX with a proposal_id (direct access)
...
// If accessed through a button click with a specific ID
elseif (isset($_GET['proposal_id']) && !$isAjaxRequest) {
    $proposal_id = intval($_GET['proposal_id']);
    // Find the proposal with matching ID in the $result object from the parent page
    if (isset($result) && $result->num_rows > 0) {
        // Save current position
        $current_position = $result->current_field;
        $result->data_seek(0);
        ...
        if ($current_position) {
            $result->data_seek($current_position);
        } else {
            $result->data_seek(0);
        }
    }
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="proposalModalLabel">Activity Proposal</h5>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="forceCloseModal('proposalModal')">Close</button>
            </div>
        </div>
    </div>
</div>
...
if (!isset($_GET['proposal_id'])) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const buttons = document.querySelectorAll('button[data-bs-target="#proposalModal"]');
